...
final class ConfigKeyMappings {
	...
	@Mod.EventBusSubscriber(modid = DeepPocketModElemental.ID, bus = Mod.EventBusSubscriber.Bus.MOD, value = Dist.CLIENT)
	...
	public static void event(TickEvent.ClientTickEvent event) {
		...
		if (clickChangePosition) {
			int index = DeepPocketConfigElemental.Client.OVERLAY_LOCATION.get().ordinal();
			index++;
			if (index >= OverlayLocation.values().length)
				index = 0;
			DeepPocketConfigElemental.Client.OVERLAY_LOCATION.set(OverlayLocation.values()[index]);
		}
		if (clickChangeDirection)
			DeepPocketConfigElemental.Client.OVERLAY_DIRECTION.set(!DeepPocketConfigElemental.Client.OVERLAY_DIRECTION.get());
	}
}
