...
class BulkCraftingScreen extends Screen {
	...
	private final @Nonnull ItemType[] recipe;
	...
	BulkCraftingScreen(@Nullable Screen backScreen, @Nonnull Pocket pocket, @Nonnull ItemType[] recipe) {
	...
	public boolean mouseClicked(double mx, double my, int button) {
		...
		if (hoverBtn0) {
			...
			count = pocket.getMaxExtract(recipe) >> 2;
			return true;
		}
		if (hoverBtn1) {
			DeepPocketUtils.playClickSound();
			count = pocket.getMaxExtract(recipe) >> 1;
			return true;
		}
		if (hoverBtn2) {
			DeepPocketUtils.playClickSound();
			count = pocket.getMaxExtract(recipe);
			return true;
		}
		if (hoverBtn3) {
			DeepPocketUtils.playClickSound();
